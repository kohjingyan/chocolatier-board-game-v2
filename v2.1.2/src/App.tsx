...
import { canDrawScheme, initialState, reduce, requiredActorId, viewingPlayerId } from './engine/game'
...
import { connectRoom, createRoomCode, isRoomCode, normalizeRoomCode } from './net/client'
import type { ClientMessage, RoomSnapshot } from './net/protocol'

const STORAGE = 'chocolatier-v2-playtest'
const INCOME_STORAGE = 'chocolatier-v2-income-overrides'
const COST_STORAGE = 'chocolatier-v2-cost-overrides'
...
function loadState(): GameState {
  try {
    ...
    if (raw) {
      ...
      if (!state.marketMode) state.marketMode = 'contribute'
...
function Setup({
  ...
  onStart: (seats: { name: string; isBot: boolean }[], marketMode: 'contribute' | 'direct') => void
}) {
  const [count, setCount] = useState(4)
  const [directDraw, setDirectDraw] = useState(false)
  const [seats, setSeats] = useState([
    { name: 'Amelie', isBot: false },
    { name: 'Bruno', isBot: true },
    { name: 'Chiara', isBot: true },
    { name: 'Dorian', isBot: true },
    { name: 'Elena', isBot: true },
    { name: 'Farid', isBot: true },
  ])
  return (
    <div className="setup panel">
      ...
      <h1>Chocolatier</h1>
      <p className="tiny">
        4 stages, 8 rounds each. Build factories, climb Research and Reputation, and finish with the
      ...
      <label className="tiny" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginTop: '0.8rem' }}>
        <input
          type="checkbox"
          checked={directDraw}
          onChange={(e) => setDirectDraw(e.target.checked)}
          style={{ width: 'auto' }}
        />
        Direct draw: skip Market Composition. Each turn draws 1× players cards from the Stage deck into the pool.
      </label>
      <div className="row" style={{ marginTop: '1rem' }}>
        <button
          type="button"
          data-testid="start-game"
          onClick={() => onStart(seats.slice(0, count), directDraw ? 'direct' : 'contribute')}
        >
          Open the atelier
        </button>
      </div>
  ...
  return (
    <aside className="rail">
      ...
      <div className="meta">
        ...
        {state.marketMode === 'direct' ? (
          <>
            <br />
            Direct draw · Stage deck {state.stageDeck.length}
          </>
        ) : null}
      </div>
        ...
        return (
          <div key={p.id} className={`player-card ${actor === p.id ? 'active' : ''}`}>
            ...
            {state.phase === 'scheme' &&
              actor &&
              actor !== p.id &&
              playerById(state, actor).schemes.some((c) => c.schemeId === 'double_agent') && (
                <div className="tiny">
                  <b>Hand</b> {p.hand.length > 0 ? p.hand.map((c) => c.name).join(', ') : 'empty'}
                </div>
              )}
            <div className="tiny">
              {p.factories.map((f) => (
        ...
        <label key={c.name} className="dev-row">
          <span>
            ...
            <span className="tiny light">
              printed {c.name === 'Extra Ingredient Slot' ? '$5/$10/$15/$20M' : money(c.cost)}
            </span>
          </span>
          <input
            type="number"
    ...
    <details className="recipe-board-wrap" open={players.length > 0} data-testid="recipe-board">
      ...
      <div className="recipe-board-scroll">
        <table className="ref-table recipe-board">
          ...
          <tbody>
            {([1, 2, 3, 4] as const).flatMap((stage) => [
        ...
        <div key={stage} style={{ marginBottom: '0.55rem' }}>
          ...
          {RECIPES.filter((r) => r.likelyStage === stage).map((r) => (
            <div key={r.id} className="tiny light">
              <b>{r.name}</b> T{r.tier} (Res L{r.tier - 1}) {money(recipePay(state, r))}
              {state.incomeOverrides?.[r.id] !== undefined && state.incomeOverrides[r.id] !== r.income
                ? ` (printed ${money(r.income)})`
                : ''}{' '}
              · {r.ingredients.map((fam) => INGREDIENT_LABEL[fam]).join(', ')} · cap {r.limit}
            </div>
          ))}
        </div>
      ))}
  ...
  return (
    <details>
      ...
      <div className="tiny light" style={{ marginBottom: '0.55rem' }}>
        ...
        Recipe Tier N unlocks at Research L(N−1) (Tier 1 is available at L0). Sea Salt needs L2 to buy from the pool.
        Masteries need Research L3 or Reputation L3. Research also grants 25% off Coffee/Almond at L1, Wild Berries/Caramel at L2, and Royal Honey at L3.
      </div>
      <div className="tiny light" style={{ marginBottom: '0.55rem' }}>
        <b>Reputation</b> — assign Junior / Senior / Master Journalists for +2 / +3 / +5 RepP. Completing a recipe on{' '}
        <b>Done assigning</b> awards 1 / 2 / 4 / 8 Rep for 3 / 4 / 5 / 6 ingredients, plus +1 only if you are first in the game to make it.
        Playing Schemes spends Rep. Levels: 5 → L1, 13 → L2, 21 → L3.
        Reputation bonus Research is +1 / +3 / +5 ResP at L1 / L2 / L3 (totals, lost if you drop).
        L1: buy one extra ingredient slot for ${REP_SLOT_L1_COST}M. L2: a second ${REP_SLOT_L2_COST}M slot, plus one extra factory for ${REP_FACTORY_COST}M. Strawberry needs L2 to buy from the pool.
        Reputation also grants the same 25% family discounts as Research, and they add together.
      </div>
      <div className="tiny light">
        <b>Most Reputable</b> — first awarded to a unique lead with at least 5 RepP. Ties do not vacate it;
        the holder keeps the title until another player has strictly more Reputation (and still at least 5),
        or the holder drops below 5. Hostile Takeover overrides the title for this turn and the next 2.
        Discount applies only to pool purchases: $1M / $1M / $2M / $3M in Stages
        1 / 2 / 3 / 4, then the track percents of the original face cost, floor $0.
    ...
    <aside className="log-rail">
      ...
      {state.effects.length > 0 && (
        <div className="tiny light" style={{ marginBottom: '0.6rem' }}>
          ...
          {state.effects
            .map((e) => {
              if (e.kind === 'monopoly') return `Monopoly ${e.recipeId} (${e.remaining})`
      ...
      <div className="log">
        ...
        <details>
          ...
          {Object.entries(SCHEME_INFO).map(([id, info]) => (
  ...
  return (
    <div className="modal-back">
      <div className="modal panel">
        ...
        {(schemeId === 'patent_lawsuit' || schemeId === 'monopoly') && (
          <select onChange={(e) => setTargets({ recipeId: e.target.value })}>
            <option value="">Your recipe</option>
            {[...new Set(player.factories.map((f) => f.recipeId).filter(Boolean))].map((id) => (
        ...
        <div className="row" style={{ marginTop: '0.8rem' }}>
          <button type="button" onClick={() => onPlay(targets)}>
  ...
  return (
    <div className="grid">
      ...
      {(player.mastery === 'jack_of_all_trades' || player.mastery === 'heritage_line') && (
        <div>
          <h3>Mastery targets</h3>
          <div className="row">
            <select
              value={player.masteryChoices.t1 ?? ''}
              onChange={(e) =>
                send({
                  type: 'SET_MASTERY_CHOICES',
                  choices: { ...player.masteryChoices, t1: e.target.value },
                })
              }
            >
              <option value="">Tier 1</option>
              {RECIPES.filter((r) => r.tier === 1).map((r) => (
                <option key={r.id} value={r.id}>
                  {r.name}
                </option>
              ))}
            ...
            <select
              value={player.masteryChoices.t2 ?? ''}
              onChange={(e) =>
                send({
                  type: 'SET_MASTERY_CHOICES',
                  choices: { ...player.masteryChoices, t2: e.target.value },
                })
              }
            >
              <option value="">Tier 2</option>
              {RECIPES.filter((r) => r.tier === 2).map((r) => (
                <option key={r.id} value={r.id}>
                  {r.name}
                </option>
              ))}
            </select>
            {player.mastery === 'jack_of_all_trades' && (
              <select
                value={player.masteryChoices.t3 ?? ''}
                onChange={(e) =>
                  send({
                    type: 'SET_MASTERY_CHOICES',
                    choices: { ...player.masteryChoices, t3: e.target.value },
                  })
                }
              >
                <option value="">Tier 3</option>
                {RECIPES.filter((r) => r.tier === 3).map((r) => (
                  <option key={r.id} value={r.id}>
                    {r.name}
                  </option>
                ))}
...
function PhaseView({
  ...
  const [schemePick, setSchemePick] = useState<keyof typeof SCHEME_INFO | null>(null)
  ...
  if (online && selfId && state.players.length > 0) {
    ...
    if (required && required !== selfId) {
      const who = playerById(state, required).name
      return (
        <div className="panel">
    ...
    return (
      <div className="panel">
        ...
        <div className="row" style={{ marginTop: '0.8rem' }}>
          <button
            type="button"
            disabled={state.openCocoa <= 0 || player.cash < 5}
            onClick={() => send({ type: 'BUY_OPEN', item: 'cocoa' })}
          >
            Open Cocoa $5M ({state.openCocoa})
          </button>
          <button
            type="button"
            disabled={state.openSugar <= 0 || player.cash < 5}
            onClick={() => send({ type: 'BUY_OPEN', item: 'sugar' })}
          >
            Open Sugar $5M ({state.openSugar})
          </button>
        </div>
      </div>
    )
      ...
      <div className="panel">
            ...
            <HoverTip key={c.uid} text={schemeTooltip(c.schemeId, state.playerCount, state.stage)}>
              <button
                ...
                onClick={() => {
                  ...
                  if (instant.includes(c.schemeId)) {
                    send({ type: 'PLAY_SCHEME', cardUid: c.uid, targets: {} })
                  } else {
                    setSchemePick(c.schemeId)
                  }
                }}
                disabled={
        ...
        {schemePick && (
          <SchemeTargeter
            ...
            schemeId={schemePick}
            onPlay={(targets) => {
              const card = actor.schemes.find((c) => c.schemeId === schemePick)
              if (card) send({ type: 'PLAY_SCHEME', cardUid: card.uid, targets })
              setSchemePick(null)
            }}
            onCancel={() => setSchemePick(null)}
          />
        )}
      </div>
    )
  ...
  if (state.phase === 'relief') {
    ...
    return (
      <div className="panel">
        ...
        <p className="tiny">
          Frozen standings. Rank 3–4 may take up to 1 package; rank 5+ may take up to 2 different packages.
          Each package costs {RELIEF_PENALTY[stage]} final-score penalty. You may take fewer or skip.
        </p>
  ...
  if (state.phase === 'assignment') {
    return (
      <div className="panel">
        <AssignmentView state={state} player={currentActor(state)} send={send} />
      </div>
    )
  }
        ...
        {state.players.map((p) => {
          ...
          return (
            <div key={p.id}>
              ...
              {money(b.research)}, reputation {money(b.reputation)}, masteries {money(b.masteries)}
            </div>
          )
        })}
...
export default function App() {
  ...
  if (liveState.phase === 'setup') {
    return (
      <div data-testid="game-root" data-phase="setup">
        <Setup
          onStart={(seats, marketMode) => send({ type: 'START', config: { seats, marketMode } })}
        />
        <OnlineEntry
  ...
  return (
    <div className="app" data-testid="game-root" data-phase={liveState.phase}>
      <PlayerRail state={liveState} selfId={online ? selfId : null} />
      <main className="main">
        {online && roomCode && (
          <p className="tiny light">
            Room {roomCode}
        ...
        {incomeHint && liveState.phase !== 'game_over' && (
          <p className="tiny light" style={{ marginTop: '0.8rem' }}>
            ...
            {' · '}
            Global recipe use:{' '}
            {RECIPES.filter((r) => producedCount(liveState, r.id) > 0)
              .map((r) => `${r.name} ${producedCount(liveState, r.id)}/${factoryLimit(r, liveState.playerCount)}`)
              .join(', ') || 'none'}
          </p>
        )}
      </main>
      <LogRail
        ...
        send={send}
        viewerId={online ? selfId : viewingPlayerId(liveState)}
      />
    </div>
  )
}
