export type BotWeights = {
  ...
  reliefMargin: number
  recipeRepScale: number
  markupShare: number
  bidRivalEdge: number
  trackProgress: number
  researchUnlock: number
}
...
export const BOT_WEIGHT_BOUNDS: { [K in keyof BotWeights]: [number, number] } = {
  ...
  bidRivalEdge: [0, 1],
  trackProgress: [0, 1.5],
  researchUnlock: [0, 1.5],
}

export const BOT_STYLES: BotStyle[] = [
  { id: 'tycoon', bid: 1.35, scheme: 0.75, deny: 1.15, rep: 0.8, temperature: 0.22 },
  { id: 'schemer', bid: 1.05, scheme: 1.45, deny: 0.85, rep: 1.3, temperature: 0.4 },
  { id: 'miser', bid: 0.88, scheme: 0.7, deny: 1.3, rep: 1, temperature: 0.18 },
  { id: 'wildcard', bid: 1.2, scheme: 1.2, deny: 1.0, rep: 1.1, temperature: 0.7 },
]
...
let training = false
// Bumped whenever any weight source changes, so the clamped results below can be reused safely.
let weightsVersion = 0
let currentCache: { version: number; value: BotWeights } | null = null
const seatCache = new Map<string, { version: number; source: BotWeights; value: BotWeights }>()
...
/**
 * Style temperatures are small next to how far apart bot scores usually are (tens of points),
 * so they are scaled up before sampling. Raise for sloppier bots, lower (or 0) for sharper ones.
 */
export const BOT_TEMPERATURE_SCALE = 6

/** Tests and training runs stay fully deterministic: every bot always plays its top choice. */
export function deterministicBots(): boolean {
...
export function currentBotWeights(): BotWeights {
  if (currentCache && currentCache.version === weightsVersion) return currentCache.value
  const value = clampBotWeights({ ...DEFAULT_BOT_WEIGHTS, ...trained, ...override })
  currentCache = { version: weightsVersion, value }
  return value
}

export function weightsFor(playerId: string): BotWeights {
  const seated = seatOverrides?.[playerId]
  if (!seated) return currentBotWeights()
  const hit = seatCache.get(playerId)
  if (hit && hit.version === weightsVersion && hit.source === seated) return hit.value
  const value = clampBotWeights(seated)
  seatCache.set(playerId, { version: weightsVersion, source: seated, value })
  return value
}