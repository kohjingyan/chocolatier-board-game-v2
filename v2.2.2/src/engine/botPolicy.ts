...
const CALL_FOR_BID_TYPE_INDEX = ACTION_TYPES.indexOf('CALL_FOR_BID')
// Where CALL_FOR_BID's one-hot slot sits in the full feature vector:
// stateFeat (24) + its position within the ACTION_TYPES block.
const CALL_FOR_BID_FEATURE_INDEX = 24 + CALL_FOR_BID_TYPE_INDEX
...
} else if (trainedPolicy?.w?.length === POLICY_DIM - 1 && CALL_FOR_BID_TYPE_INDEX >= 0) {
  // Saved before CALL_FOR_BID existed. ACTION_TYPES sits in the middle of the
  // feature vector (stateFeat, then typeFeat, then the per-action detail
  // features), so the new weight must be *inserted* at CALL_FOR_BID's slot,
  // not appended at the end — appending would misalign every detail-feature
  // weight that comes after it (bid amount, card cost/kind, etc.).
  const w = [
    ...trainedPolicy.w.slice(0, CALL_FOR_BID_FEATURE_INDEX),
    0,
    ...trainedPolicy.w.slice(CALL_FOR_BID_FEATURE_INDEX),
  ]
  policy = {
    dim: POLICY_DIM,
    w,
...
function bidAmounts(cash: number): number[] {
  const raw = [
    ...
    Math.min(cash, 8),
    Math.min(cash, 12),
    Math.round(cash * 0.8), // strong bid, but never the whole stack
  ]
...
export function legalBotActions(state: GameState): { player: Player; actions: Action[] } | null {
  ...
  if (state.phase === 'contribute') {
    ...
    if (!player.hasCalledBid && state.stage > 1) actions.push({ type: 'CALL_FOR_BID', actorId: player.id })
    return { player, actions }
  }
  if (state.phase === 'private_viewing') {
