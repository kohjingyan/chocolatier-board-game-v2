...
import { isValidMarkup, markupFor, markupsLeft } from './markup'
...
/**
 * Snapshot every player's running total so the score chart can show who led when.
 * Snapshots are keyed by stage + round; recording the same key again replaces the earlier one
 * (used to fold end-of-stage relief into that stage's last point). Round 0 is the starting line.
 */
function recordScores(state: GameState, round: number = state.round) {
  if (!state.scoreHistory) state.scoreHistory = []
  const scores: Record<string, number> = {}
  for (const row of finalScores(state)) scores[row.id] = row.total
  const entry = { stage: state.stage, round, scores }
  const i = state.scoreHistory.findIndex((h) => h.stage === entry.stage && h.round === entry.round)
  if (i >= 0) state.scoreHistory[i] = entry
...
function revealPool(state: GameState) {
  ...
  for (const id of state.turnOrder) {
    ...
    cards.push(card)
    const markup = state.contributedMarkup?.[id] ?? 0
    if (markup > 0) state.poolMarkup[card.uid] = { sellerId: id, amount: markup }
  }
  state.pool = shuffle(cards, state)
  const label = (c: GameCard) => {
    const entry = state.poolMarkup[c.uid]
    return entry ? `${c.name} (+$${entry.amount}M markup)` : c.name
  }
  log(state, `Market revealed: ${state.pool.map(label).join(', ')}.`)
...
function advanceStage(state: GameState) {
  recordScores(state, 8) // fold this stage's relief picks into its last data point
...
function apply(state: GameState, action: Action) {
  ...
  if (action.type === 'CONTRIBUTE') {
    ...
    const markup = action.markup ?? 0
    if (markup !== 0 && (!isValidMarkup(markup) || markupsLeft(player) <= 0)) return
    state.contributed[player.id] = action.cardUid
    if (markup > 0) {
      if (!state.contributedMarkup) state.contributedMarkup = {}
      state.contributedMarkup[player.id] = markup
      player.markupsUsed = (player.markupsUsed ?? 0) + 1
    }
  ...
  if (action.type === 'CALL_FOR_BID') {
    const phase = state.phase === 'curtain' ? state.resumePhase : state.phase
    if (phase !== 'contribute') return
    const inferred = firstUnfinished(state, (p) => Boolean(state.contributed[p.id]))?.id
    const player = resolveActor(state, action, inferred)
    if (player.hasCalledBid) return
    player.hasCalledBid = true
    log(state, `${player.name} calls for a bid! Turn order is redrawn.`)
    // Contributions made so far this round are void and everyone contributes again after the bid,
    // so give back any markup uses that were already spent.
    for (const [id, amount] of Object.entries(state.contributedMarkup ?? {})) {
      if (amount > 0) {
        const p = playerById(state, id)
        p.markupsUsed = Math.max(0, (p.markupsUsed ?? 0) - 1)
      }
    }
    state.contributedMarkup = {}
    state.contributed = {}
    ...
    state.bidRound = 'main'
    if (state.playMode === 'online') showSharedPhase(state, 'bid')
    else showPhase(state, 'bid', state.turnOrder[0])
    return
  }
  ...
  if (action.type === 'BUY_POOL') {
    ...
    const markup = markupFor(state, player, card)
    if (player.cash < price + markup) return
    pay(player, price)
    takeCard(state.pool, card.uid)
    giveCard(state, player, card)
    let paidNote = ''
    const listed = state.poolMarkup?.[card.uid]
    if (markup > 0 && listed) {
      const seller = playerById(state, listed.sellerId)
      pay(player, markup)
      seller.cash += markup
      paidNote = ` plus $${markup}M markup to ${seller.name}`
    } else if (listed) {
      paidNote = ` (own card, $${listed.amount}M markup waived)`
    }
    delete state.poolMarkup[card.uid]
    log(state, `${player.name} buys ${card.name} for $${price}M${paidNote}.`)
    nextPurchase(state)
    return
  }
