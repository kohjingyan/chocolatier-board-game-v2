...
export class RoomLogic {
  ...
  constructor(
    private readonly emit: (connId: string, msg: ServerMessage) => void,
    private readonly allIds: () => string[],
    private readonly persist?: (data: PersistedRoom) => void,
  ) {}

  hydrate(data: PersistedRoom) {
    if (!data || !Array.isArray(data.seats)) return
    this.hostId = typeof data.hostId === 'string' ? data.hostId : ''
    this.seats = data.seats
    this.game = data.game ?? null
    if (this.game) this.syncUids(this.game)
  }

  persisted(): PersistedRoom {
    return { hostId: this.hostId, seats: this.seats, game: this.game }
  }
  ...
  private handle(msg: ClientMessage, connId: string) {
    ...
    if (msg.type === 'action') {
      ...
      this.game = flushBots(reduce(this.game, playerId ? { ...action, actorId: playerId } : action))
      this.broadcast()
    }
  }
