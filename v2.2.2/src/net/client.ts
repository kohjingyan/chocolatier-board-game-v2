...
const ROOM_CACHE_KEY = 'chocolatier-room-cache'

type CachedRoom = { code: string; room: RoomSnapshot; you: string }

export function readCachedRoom(code: string): CachedRoom | null {
  try {
    const raw = sessionStorage.getItem(ROOM_CACHE_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw) as CachedRoom
    if (parsed?.code !== normalizeRoomCode(code) || !parsed.room) return null
    return parsed
  } catch {
    return null
  }
...
export function writeCachedRoom(code: string, room: RoomSnapshot, you: string) {
  try {
    sessionStorage.setItem(ROOM_CACHE_KEY, JSON.stringify({ code: normalizeRoomCode(code), room, you }))
  } catch {
    /* ignore quota */
  }
}

export function clearCachedRoom() {
  try {
