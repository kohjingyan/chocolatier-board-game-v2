...
function loadState(): GameState {
  try {
    ...
    if (raw) {
      ...
      if (!state.contributedMarkup) state.contributedMarkup = {}
      if (!state.poolMarkup) state.poolMarkup = {}
      ...
      for (const p of state.players) {
        ...
        if (p.markupsUsed === undefined) p.markupsUsed = 0
  ...
  return (
    <HoverTip text={tip}>
      <button
        ...
        style={props.markup ? { position: 'relative', overflow: 'visible' } : undefined}
      ...
      >
        ...
        {props.markup ? (
          <span
            ...
            data-testid="markup-badge"
            title={`Markup: +${money(props.markup)} paid to the seller`}
            style={{
              position: 'absolute',
              right: -8,
              bottom: -8,
...
function ContributePanel({
  ...
  const left = markupsLeft(player)
  const active = left > 0 ? markup : 0
  const options = Array.from({ length: MARKUP_MAX - MARKUP_MIN + 1 }, (_, i) => MARKUP_MIN + i)
  return (
    <div className="panel">
      <h2>Market composition — {player.name}</h2>
      <p className="tiny">Choose one card from your hand for the shared pool.</p>
      <div data-testid="markup-picker" style={{ margin: '0.6rem 0' }}>
        <p className="tiny" style={{ marginBottom: '0.3rem' }}>
          <b>Markup</b> (optional): added to the buyer's price and paid to you if someone buys your card.{' '}
          <b>
            {left} of {MARKUPS_PER_STAGE}
          </b>{' '}
          markups left this stage.
        </p>
        <div className="row" style={{ flexWrap: 'wrap', gap: '0.3rem' }}>
          <button
            type="button"
            className={active === 0 ? '' : 'secondary'}
            aria-pressed={active === 0}
            data-testid="markup-none"
            onClick={() => setMarkup(0)}
          >
            None
          </button>
          {options.map((n) => (
            <button
              key={n}
              type="button"
              className={active === n ? '' : 'secondary'}
              aria-pressed={active === n}
              data-testid={`markup-${n}`}
              disabled={left <= 0}
              onClick={() => setMarkup(n)}
            >
              +{n}
        ...
        <p className="tiny" style={{ marginTop: '0.3rem' }}>
          {active > 0
            ? `Your card will carry a +$${active}M markup. Click a card to contribute it.`
            : 'No markup selected.'}
        </p>
      </div>
      <div className="cards">
        {player.hand.map((c) => (
          <CardView
            ...
            state={state}
            card={c}
            markup={active > 0 ? active : undefined}
            testId="contribute-card"
            onClick={() => send({ type: 'CONTRIBUTE', cardUid: c.uid, markup: active > 0 ? active : undefined })}
          />
        ))}
      </div>
      <div className="row" style={{ marginTop: '1rem', alignItems: 'center', gap: '0.6rem' }}>
        <button
          type="button"
          className="secondary"
          data-testid="call-for-bid"
          disabled={player.hasCalledBid}
          onClick={() => send({ type: 'CALL_FOR_BID' })}
        >
          {player.hasCalledBid ? 'Bid already called' : 'Call for bid'}
        </button>
        <p className="tiny light" style={{ margin: 0 }}>
          {player.hasCalledBid
            ? 'You have used your one-time call for this game.'
            : 'Once per game: skip contributing and force a fresh bid for turn order instead.'}
        </p>
      </div>
    </div>
  )
...
const PLAYER_COLORS = ['#b03a2e', '#1f78b4', '#2e8b57', '#c9891a']
...
function computeLeadInfo(state: GameState) {
  const history = state.scoreHistory ?? []
  const ids = state.players.map((p) => p.id)
  const leaderAt: (string | null)[] = []
  const events: LeadEvent[] = []
  const roundsLed: Record<string, number> = {}
  let prev: string | null = null
  history.forEach((h, index) => {
    if (h.round === 0) {
      leaderAt.push(null) // everyone starts level
      return
    }
    const top = Math.max(...ids.map((id) => h.scores[id] ?? 0))
    const leaders = ids.filter((id) => (h.scores[id] ?? 0) === top)
    let leader: string | null = null
    if (leaders.length === 1) leader = leaders[0]
    else if (prev && leaders.includes(prev)) leader = prev // a tie keeps the current leader
    leaderAt.push(leader)
    if (!leader) return
    roundsLed[leader] = (roundsLed[leader] ?? 0) + 1
    if (leader !== prev) {
      const margin = prev ? (h.scores[leader] ?? 0) - (h.scores[prev] ?? 0) : 0
      events.push({ index, stage: h.stage, round: h.round, leaderId: leader, previousId: prev, margin })
      prev = leader
    }
  })
  return { leaderAt, events, roundsLed }
}
...
function ScoreChart({ state }: { state: GameState }) {
  const history = state.scoreHistory ?? []
  if (history.length < 2) {
    return <p className="tiny">The chart starts after the first round's income is paid.</p>
  }
  const { leaderAt, events, roundsLed } = computeLeadInfo(state)
  const W = 760
  const H = 300
  const ml = 46
  const mr = 16
  ...
  const all = history.flatMap((h) => state.players.map((p) => h.scores[p.id] ?? 0))
  const rawLo = Math.min(...all)
  const rawHi = Math.max(...all)
  const step = niceStep(Math.max(1, (rawHi - rawLo) / 4))
  ...
  // Stage bands: from the midpoint before a stage's first point to the midpoint after its last.
  const bands: { stage: number; from: number; to: number }[] = []
  history.forEach((h, i) => {
    const last = bands[bands.length - 1]
    if (last && last.stage === h.stage) last.to = i
    else bands.push({ stage: h.stage, from: i, to: i })
  })
  const half = plotW / (n - 1) / 2
  const bandX = (i: number, edge: 'from' | 'to') =>
    Math.min(ml + plotW, Math.max(ml, x(i) + (edge === 'from' ? -half : half)))

  const nameOf = (id: string) => playerById(state, id).name
  const colorOf = (id: string) => PLAYER_COLORS[state.players.findIndex((p) => p.id === id) % PLAYER_COLORS.length]
  const stageLabel = (stage: number, round: number) => (round === 0 ? 'Start' : `Stage ${stage}, round ${round}`)
  const current = leaderAt[leaderAt.length - 1]

  return (
    <div data-testid="score-chart">
      <div className="row" style={{ flexWrap: 'wrap', gap: '0.8rem', marginBottom: '0.3rem' }}>
        {state.players.map((p) => (
          <span key={p.id} className="tiny" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 12, height: 3, background: colorOf(p.id), display: 'inline-block', borderRadius: 2 }} />
            {p.name}
          </span>
        ))}
        <span className="tiny light">Ringed dot = leader that round · dashed line = lead change</span>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', minWidth: 480, height: 'auto' }} role="img" aria-label="Total score by round for each player">
          {bands.map((b, k) => (
            <g key={`${b.stage}-${k}`}>
              {b.stage % 2 === 0 && (
                <rect x={bandX(b.from, 'from')} y={mt} width={bandX(b.to, 'to') - bandX(b.from, 'from')} height={plotH} fill="rgba(60,40,20,0.06)" />
              )}
              <text x={(bandX(b.from, 'from') + bandX(b.to, 'to')) / 2} y={mt - 12} textAnchor="middle" fontSize="11" fontWeight="700" fill="#5b4636">
                Stage {b.stage}
              </text>
            </g>
          ))}
          {ticks.map((v) => (
            <g key={v}>
              <line x1={ml} x2={W - mr} y1={y(v)} y2={y(v)} stroke="rgba(60,40,20,0.15)" />
              <text x={ml - 6} y={y(v) + 3} textAnchor="end" fontSize="10" fill="#5b4636">
                {v}
              </text>
            </g>
          ))}
          {history.map((h, i) => (
            <text key={`${h.stage}-${h.round}`} x={x(i)} y={H - mb + 14} textAnchor="middle" fontSize="10" fill="#5b4636">
              {h.round === 0 ? '0' : h.round}
            </text>
          ))}
          <text x={ml + plotW / 2} y={H - 4} textAnchor="middle" fontSize="10" fill="#5b4636">
            Round (0 = start)
          </text>
          {events.map((e) => (
            <line key={`ev-${e.index}`} x1={x(e.index)} x2={x(e.index)} y1={mt} y2={mt + plotH} stroke={colorOf(e.leaderId)} strokeDasharray="4 3" opacity="0.7" />
          ))}
          {state.players.map((p) => (
            <polyline
              key={p.id}
              ...
              points={history.map((h, i) => `${x(i)},${y(h.scores[p.id] ?? 0)}`).join(' ')}
            />
          ))}
          {state.players.map((p) =>
            history.map((h, i) => (
              <g key={`${p.id}-${i}`}>
                <circle cx={x(i)} cy={y(h.scores[p.id] ?? 0)} r={3.2} fill={colorOf(p.id)}>
                  <title>{`${p.name} — ${stageLabel(h.stage, h.round)}: ${h.scores[p.id] ?? 0}`}</title>
                </circle>
                {leaderAt[i] === p.id && (
                  <circle cx={x(i)} cy={y(h.scores[p.id] ?? 0)} r={7} fill="none" stroke="#2b1810" strokeWidth="1.6" />
                )}
              </g>
            )),
          )}
      ...
      <div className="tiny" data-testid="lead-changes" style={{ marginTop: '0.4rem' }}>
        <b>Lead history</b>
        {events.length === 0 ? (
          <div>No one has pulled ahead yet.</div>
        ) : (
          events.map((e) => (
            <div key={`le-${e.index}`}>
              <span style={{ color: colorOf(e.leaderId), fontWeight: 700 }}>●</span> {stageLabel(e.stage, e.round)}:{' '}
              {e.previousId
                ? `${nameOf(e.leaderId)} overtakes ${nameOf(e.previousId)} (+${e.margin})`
                : `${nameOf(e.leaderId)} takes the lead`}
            </div>
          ))
        )}
        <div style={{ marginTop: '0.3rem' }}>
          <b>Rounds in the lead:</b>{' '}
          {state.players.map((p) => `${p.name} ${roundsLed[p.id] ?? 0}`).join(' · ')}
          {current ? ` · Currently ahead: ${nameOf(current)}` : ''}
        </div>
      </div>
    </div>
  )
...
function ScoreBoard({ state, hideTable }: { state: GameState; hideTable?: boolean }) {
  const rows = finalScores(state)
    .map((r) => {
      // While bids are sealed, add each player's bid back so the table doesn't leak it.
      const cash = visibleCash(state, playerById(state, r.id))
      return { ...r, total: r.total - r.cash + cash, cash }
    })
    .sort((a, b) => b.total - a.total)
  const top = rows[0]?.total ?? 0
  return (
    <details className="panel" open data-testid="scoreboard" style={{ marginTop: '0.8rem' }}>
      <summary style={{ cursor: 'pointer' }}>
        <b>{hideTable ? 'Score history' : 'Live scores'}</b>
        <span className="tiny light"> · Stage {state.stage}, round {Math.min(state.round, 8)}</span>
      </summary>
      {!hideTable && (
        <>
          <div style={{ overflowX: 'auto', marginTop: '0.5rem' }}>
            <table className="score-table">
              ...
              <tbody>
                {rows.map((s) => (
                  <tr key={s.id} className={s.total === top ? 'win' : ''}>
                    <td>{rows.findIndex((r) => r.total === s.total) + 1}</td>
                    <td>{s.name}</td>
                    <td>{s.cash}</td>
                    <td>{s.research}</td>
                    <td>{s.reputation}</td>
                    ...
                    <td>{s.total === top ? '—' : `−${top - s.total}`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="tiny light" style={{ marginTop: '0.3rem' }}>
            Scores as they would stand if the game ended now. Production counts 3× your last paid income, so it updates
            after each income step.
          </p>
        </>
      )}
...
function CallForBidBanner({ state }: { state: GameState }) {
  const [active, setActive] = useState<{ id: number; name: string } | null>(null)
  const lastSeenId = useRef<number | null>(null)

  useEffect(() => {
    if (lastSeenId.current === null) {
      // Don't fire for log entries that already existed before this mounted.
      lastSeenId.current = state.logSeq
      return
    }
    if (state.logSeq <= lastSeenId.current) return
    const fresh = state.log.filter((e) => e.id > (lastSeenId.current ?? 0))
    lastSeenId.current = state.logSeq
    const call = [...fresh].reverse().find((e) => e.text.includes('calls for a bid'))
    if (call) {
      const name = call.text.split(' calls for a bid')[0]
      setActive({ id: call.id, name })
    }
    ...
    <>
      <style>{`
        ...
        .bid-call-banner {
          ...
          animation: bidCallPop 2.2s ease-out forwards;
      ...
      `}</style>
      {active && (
        <div className="bid-call-overlay">
          <div className="bid-call-banner" key={active.id}>
            <p className="headline">Bid called!</p>
            <p className="subline">{active.name} shakes up the turn order</p>
          </div>
        </div>
      )}
    </>
...
function PhaseView({
  ...
  if (state.phase === 'contribute') {
    return <ContributePanel key={`${player.id}-${state.stage}-${state.round}`} state={state} player={player} send={send} />
  }
    ...
    return (
      <div className="panel">
        ...
        <p>
          ...
          {state.pool
            .map((c) => {
              const m = listedMarkup(state, c)
              return m > 0 ? `${c.name} (+${m})` : c.name
            })
            .join(', ')}
        </p>
        <div className="row">
      ...
      <div className="panel">
        ...
        <div className="cards">
          {state.pool.map((c) => {
            ...
            const markup = markupFor(state, player, c)
            const price = poolPrice(state, player, c) + markup
            const ownCard = state.poolMarkup?.[c.uid]?.sellerId === player.id
            return (
              <div key={c.uid}>
                <CardView state={state} card={c} locked={!obtain} price={obtain ? price : undefined} markup={listedMarkup(state, c)} />
                {ownCard && listedMarkup(state, c) > 0 && (
                  <div className="tiny">Your markup — you pay none on your own card.</div>
                )}
                <div className="row" style={{ marginTop: '0.8rem' }}>
                  <button
                    type="button"
